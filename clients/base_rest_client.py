...
class BaseRestAPIClient(ABC):
    ...
    async def patch(self, endpoint: str, response_model: Type[BaseModel], auth_required: bool = False,
                    **kwargs: Any) -> Any:
        json_payload = await self._prepare_payload(auth_required=auth_required, **kwargs)
        response = await self._make_request(method='PATCH', endpoint=endpoint, json_data=json_payload)
        return response_model.model_validate(response.json())
